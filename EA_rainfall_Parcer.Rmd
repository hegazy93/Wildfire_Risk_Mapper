...
stations <- read_csv("http://environment.data.gov.uk/flood-monitoring/id/stations.csv", show_col_types = FALSE) |>
  ...
  dplyr::select(stationReference, lat, long)
...
range <- seq(as.Date("2024/03/12"), as.Date("2024/03/15"), by = "days")
out_url <- apply(expand.grid(base_url, range, ".csv"), 1, paste, collapse = '')
...
rainfall_hist <- readr::read_csv((r"(2_processed_data\EA_daily_rainfall_stations.csv)"), col_types = cols(stationReference = col_character(), date = col_date(), daily_rainfall =col_double()))
...
for (i in 1:length(rainfall_list)){
  ...
  output_file <- file.path(r"(2_processed_data\interpolated_rainfall)",
                           paste0(as.character(names(rainfall_list[i])), ".tif"))
...
rainfall_stack <- rast(rainfall_stack)

writeRaster(rainfall_stack, filename = r"(2_processed_data\daily_rainfall.tif)", filetype = "GTiff", overwrite=TRUE)
```
...
```{r}
soil_moisture <- terra::rast(r"(2_processed_data\soil_wetness.tif)")[[2]]
crs(rainfall_stack) <- crs(soil_moisture)
ext(rainfall_stack) <- ext(soil_moisture)

calculate_ari <- function(rainfall_raster,soil_raster) {
  antecedent_rainfall <- terra::rast(ncols = ncol(rainfall_raster), nrows = nrow(rainfall_raster), 
                                     crs =crs(rainfall_raster), extent = ext(rainfall_raster), 
                                     nlyrs = nlyr(rainfall_raster))
  for (i in 1:nlyr(rainfall_raster)){
    start_index <- max(1, i - 10)
    end_index <- i
    cumulative_precipitation <- rainfall_raster[[1]]
    values(cumulative_precipitation) <- 0
    
    for (j in start_index:end_index) {
      contribution <- rainfall_raster[[j]] * soil_raster ^ (i - j)
      cumulative_precipitation <- cumulative_precipitation + contribution
      }
    antecedent_rainfall[[i]] <- cumulative_precipitation
    names(antecedent_rainfall[[i]]) <- names(rainfall_raster[[i]])
  }
  return(antecedent_rainfall)
}

antecedent_rainfall <- calculate_ari(rainfall_stack, soil_moisture)
  
plot(antecedent_rainfall[[30]])
varnames(antecedent_rainfall)<- "ARI"
writeRaster(antecedent_rainfall, filename = r"(2_processed_data\ari_runoff_10.tif)", filetype = "GTiff", overwrite=TRUE)
