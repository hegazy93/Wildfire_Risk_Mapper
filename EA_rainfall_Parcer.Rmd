...
calculateAPIList <- function(df_list, rainfall_column, n) {
  ...
  df_list_with_api <- lapply(seq_along(df_list), function(i) {
    ...
    df <- subset(df, date > as.Date("2023-08-31"))
...
idw_interpolation <- function(points_sf, polygon_sf, value_column) {
  
  # Create a spatial points data frame from the points sf
  points_spdf <- as(points_sf, "Spatial")
  
  # Create a gstat object for IDW interpolation
  idw_model <- gstat::idw(as.formula(paste(value_column, "~ 1")), points_spdf, polygon_sf)
  
  # Interpolate values to the polygon sf
  interpolated_values <- predict(idw_model, newdata = polygon_sf)
  
  return(interpolated_values)
}

polygon_sf <- st_read(r"(2_processed_data\landcover_MRSDNM.geojson)")

points_sf <- api_sf[[40]] %>%
  drop_na() %>%
  st_intersection(polygon_sf)

# Specify the column containing values to be interpolated
value_column <- "API"

# Perform IDW interpolation
interpolated_values <- idw_interpolation(points_sf, polygon_sf, value_column)

# Add interpolated values to the polygon sf
polygon_sf <- cbind(polygon_sf, interpolated_values)
